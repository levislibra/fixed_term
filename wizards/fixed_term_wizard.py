...
class FixedTermLineWizard(models.TransientModel):
    ...
    def recalculate_fixed_term_line(self):
        print "Execute ************-------------------"
        print self.execute_precancelable
        print self.new_date_maturity
        print self.date_maturity
        if self.execute_precancelable:
            if self.new_date_maturity <= self.date and self.new_date_maturity >= self.date_maturity:
                raise ValidationError('La nueva fecha debe ser mayor a la fecha de inicio y menor a la fecha de vencimiento.')
                print "Entro en el validateError"
            else:
                print "Entro en el elseeeeeeeeeeeeee"
...
class FixedTermValidateWizard(models.TransientModel):
    ...
    def validate_fixed_term_line(self):
        self.fixed_term_id.validate_fixed_term_line()